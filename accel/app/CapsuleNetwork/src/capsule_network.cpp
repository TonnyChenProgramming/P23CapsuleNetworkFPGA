...
static void debug_float_buffer(const char* name, const float* data, int size, int print_count = 16)
{
    ...
    if (data == nullptr) {
        ...
        std::cout << "===== END DEBUG FLOAT BUFFER =====" << std::endl;
        return;
    }
...
static void debug_primary_capsule_norms(const char* name, const float* data,
                                        int num_capsules, int dim_capsule)
{
    std::cout << "===== PRIMARY CAPSULE NORM DEBUG: " << name << " =====" << std::endl;

    float min_norm = 1e30f;
    float max_norm = 0.0f;
    int over_one_count = 0;
    int nan_count = 0;
    int inf_count = 0;
    ...
    for (int cap = 0; cap < num_capsules; ++cap) {
        ...
        for (int d = 0; d < dim_capsule; ++d) {
            float v = data[cap * dim_capsule + d];
        ...
        if (norm > 1.0f) over_one_count++;

        if (cap < 10) {
            std::cout << "capsule[" << cap << "] norm = " << norm << std::endl;
        }
    }

    std::cout << "min_norm = " << min_norm << std::endl;
    std::cout << "max_norm = " << max_norm << std::endl;
    std::cout << "capsules with norm > 1 = "
              << over_one_count << " / " << num_capsules << std::endl;
    std::cout << "nan_count = " << nan_count << std::endl;
    std::cout << "inf_count = " << inf_count << std::endl;
    std::cout << "===== END PRIMARY CAPSULE NORM DEBUG =====" << std::endl;
}
...
void runCapsuleNetwork(vart::RunnerExt *runner, uint32_t num_images, const xir::Subgraph *subgraph, int digitcaps_sw_imp, const string image_path, string label_path, const string weight_path, int verbose)
{
    ...
    std::cout << "\n===== DEBUG: Tensor metadata =====" << std::endl;

    std::cout << "Input tensor name: " << inputTensors[0]->get_name() << std::endl;
    std::cout << "Input shape: ";
    for (auto d : inputTensors[0]->get_shape()) {
        std::cout << d << " ";
    }
    std::cout << std::endl;
    std::cout << "Input element_num: " << inputTensors[0]->get_element_num() << std::endl;
    std::cout << "Input bit width: " << inputTensors[0]->get_data_type().bit_width << std::endl;
    std::cout << "Input type enum: " << inputTensors[0]->get_data_type().type << std::endl;

    std::cout << "Output tensor name: " << outputTensors[0]->get_name() << std::endl;
    std::cout << "Output shape: ";
    for (auto d : outputTensors[0]->get_shape()) {
        std::cout << d << " ";
    }
    std::cout << std::endl;
    std::cout << "Output element_num: " << outputTensors[0]->get_element_num() << std::endl;
    std::cout << "Output bit width: " << outputTensors[0]->get_data_type().bit_width << std::endl;
    ...
    std::cout << "inSize from shapes = " << inSize << std::endl;
    std::cout << "outSize from shapes = " << outSize << std::endl;
    std::cout << "batchSize = " << batchSize << std::endl;
    std::cout << "real input elements per batch = "
            << inputTensors[0]->get_element_num() / batchSize << std::endl;
    std::cout << "real output elements per batch = "
            << outputTensors[0]->get_element_num() / batchSize << std::endl;
    std::cout << "allocated primcaps_output floats = "
            << batchSize * outSize << std::endl;
    std::cout << "required output elements = "
            << outputTensors[0]->get_element_num() << std::endl;
    std::cout << "===== END SIZE DEBUG =====" << std::endl;
	...
	debug_float_buffer("imageInputs after load_mnist_images",
                   imageInputs,
                   batchSize * inSize,
                   32);
	...
	std::cout << "===== WEIGHTS DEBUG =====" << std::endl;
	std::cout << "weights.size() = " << weights.size() << std::endl;

	if (!weights.empty()) {
		debug_float_buffer("weights first part",
						weights.data(),
						std::min((int)weights.size(), 128),
						32);
	}

	if (weights.size() != 1474560) {
		std::cout << "WARNING: expected DigitCaps weights size 1474560, got "
	...
	for (unsigned int n = 0; n < num_images; n += batchSize)
	{
		...
		debug_float_buffer(
			"imageInputs before DPU execute",
			&imageInputs[n * inSize],
			runSize * inSize,
			32
		);

		debug_float_buffer("primcaps_output before DPU execute",
						primcaps_output,
						batchSize * outSize,
						32);
        ...
        std::cout << "===== BEFORE EXECUTE SIZE CHECK =====" << std::endl;
        std::cout << "outSize from shapes = " << outSize << std::endl;
        std::cout << "batchSize = " << batchSize << std::endl;
        std::cout << "allocated primcaps_output floats = "
                << batchSize * outSize << std::endl;

        std::cout << "real output tensor element_num = "
                << outputTensors[0]->get_element_num() << std::endl;

        std::cout << "real output tensor bit width = "
                << outputTensors[0]->get_data_type().bit_width << std::endl;

        std::cout << "fake output tensor element_num = "
                << batchTensors.back()->get_element_num() << std::endl;

        std::cout << "fake output tensor bit width = "
                << batchTensors.back()->get_data_type().bit_width << std::endl;

        std::cout << "primcaps_output address = "
                << static_cast<void*>(primcaps_output) << std::endl;
        std::cout << "===== END BEFORE EXECUTE SIZE CHECK =====" << std::endl;
		// Run DPU
		...
		debug_float_buffer("primcaps_output before CPU squash",
						primcaps_output,
						batchSize * outSize,
						64);
		...
		for (unsigned int i = 0; i < runSize; i++)
		{
			...
			if (digitcaps_sw_imp)
			{
				...
				debug_float_buffer(
					"prediction_data after software dynamic_routing",
					prediction_data,
					DIGIT_CAPS_NUM_DIGITS * DIGIT_CAPS_DIM_CAPSULE,
					32
				);
			...
			else
			{
				...
				debug_float_buffer(
					"prediction_data after hardware DigitCaps",
					prediction_data,
					DIGIT_CAPS_NUM_DIGITS * DIGIT_CAPS_DIM_CAPSULE,
					32
				);
