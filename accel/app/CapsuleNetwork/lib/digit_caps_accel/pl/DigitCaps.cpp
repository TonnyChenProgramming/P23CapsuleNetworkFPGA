...
#pragma HLS INTERFACE mode=s_axilite port=return     bundle=control
	...
	// Keep a tiny HLS-resettable state element so the generated ap_rst_n/system
	// reset has something explicit to reset. Do NOT reset huge buffers this way.
	static ap_uint<1> reset_guard = 0;
#pragma HLS RESET variable=reset_guard

	volatile int reset_local = reset;
	reset_guard = (reset_local != 0);

	// Always clear external output at kernel entry. This prevents stale output
	// if reset-only is requested or if an early return/path is added later.
	clear_prediction(prediction);

	if (reset_local != 0)
	{
		// Reset-only software path.
		// Local memories are intentionally not scrubbed here. They are not static,
		// and the normal path below clears/overwrites everything it depends on.
		return;
	}
...
#pragma HLS BIND_STORAGE variable=squashed_v type=ram_1wnr impl=lutram
	...
	// Burst read input into local array. primary_caps is fully overwritten.
	memcpy(primary_caps, (const fixed_t *)input,
		   DIGIT_CAPS_INPUT_CAPSULES * DIGIT_CAPS_INPUT_DIM_CAPSULE * sizeof(fixed_t));
...
#pragma HLS BIND_STORAGE variable=coupling_c type=ram_1wnr impl=lutram
    // softmax partition, kept from deployed version
...
#pragma HLS BIND_STORAGE variable=output_agreement type=ram_1wnr impl=lutram
	...
	// Critical reset for routing state:
	// coupling_b is updated with += during routing, so it MUST start from zero
	// on every inference. This was the dangerous accumulator state.
	clear_routing_logits(coupling_b);
	clear_routing_scratch(coupling_c, output_agreement);

	// weighted_input_u is fully overwritten by apply_weights().
	apply_weights(primary_caps, weights, weighted_input_u);

	for (uint32_t i = 0; i < DIGIT_CAPS_ROUTING_ITERATIONS; ++i)
	{
		// coupling_c and output_agreement are fully assigned later, but clearing
		// them is cheap compared with the huge weighted_input buffer and avoids
		// stale scratch state if the code changes later.
		clear_routing_scratch(coupling_c, output_agreement);

		softmax(coupling_b, coupling_c);

		// sum_of_products_s is fully assigned by sum_of_products(), not += into
		// old output_mat values.
		sum_of_products(weighted_input_u, coupling_c, sum_of_products_s);

		// squashed_v is fully assigned by squash().
		...
		if (i < DIGIT_CAPS_ROUTING_ITERATIONS - 1)
		{
			...
			// Intentional within-inference routing update. Safe because
			// coupling_b was cleared at kernel entry.
			add(output_agreement, coupling_b);
		}
	}

	memcpy(prediction, (const fixed_t *)squashed_v,
		   DIGIT_CAPS_NUM_DIGITS * DIGIT_CAPS_DIM_CAPSULE * sizeof(fixed_t));
}
}
...
static void softmax(fixed_t *mat_b, fixed_t *mat_c)
{
	...
	for (uint32_t i = 0; i < DIGIT_CAPS_INPUT_CAPSULES; ++i)
	{
		...
		{
			fixed_t entry = (fixed_t)expf((float)mat_b[i + j * DIGIT_CAPS_INPUT_CAPSULES]);
...
static void squash(fixed_t *input_mat, fixed_t *squash_mat)
{
	...
	for (uint32_t i = 0; i < DIGIT_CAPS_NUM_DIGITS; ++i)
	{
		...
		scale = squared_norm / ((fixed_t)1.0 + squared_norm) /
				(fixed_t)sqrtf((float)(squared_norm + (fixed_t)1e-7));
