...
void dynamic_routing(fixed_t *input, fixed_t *weights, fixed_t *prediction)
{
	...
	// Dynamic routing must start with b_ij = 0.
	for (uint32_t idx = 0;
		idx < DIGIT_CAPS_NUM_DIGITS * DIGIT_CAPS_INPUT_CAPSULES;
		++idx)
	{
		coupling_b[idx] = 0;
		coupling_c[idx] = 0;
		output_agreement[idx] = 0;
	}

	for (uint32_t idx = 0;
		idx < DIGIT_CAPS_NUM_DIGITS * DIGIT_CAPS_DIM_CAPSULE;
		++idx)
	{
		sum_of_products_s[idx] = 0;
		squashed_v[idx] = 0;
	}
	...
	{
		// Clear accumulation buffer every routing iteration.
		for (uint32_t idx = 0;
			idx < DIGIT_CAPS_NUM_DIGITS * DIGIT_CAPS_DIM_CAPSULE;
			++idx)
		{
...
static void softmax(fixed_t *mat_b, fixed_t *mat_c)
{
	...
	for (uint32_t i = 0; i < DIGIT_CAPS_INPUT_CAPSULES; ++i)
	{
		...
		for (uint32_t j = 0; j < DIGIT_CAPS_NUM_DIGITS; ++j)
		{
			...
			fixed_t entry = fixed_exp(mat_b[i + j * DIGIT_CAPS_INPUT_CAPSULES]);
...
static void squash(fixed_t *input_mat, fixed_t *squash_mat)
{
	...
	for (uint32_t i = 0; i < DIGIT_CAPS_NUM_DIGITS; ++i)
	{
		...
		scale = squared_norm / ((fixed_t)1.0 + squared_norm) / fixed_sqrt(squared_norm + (fixed_t)1e-7);
